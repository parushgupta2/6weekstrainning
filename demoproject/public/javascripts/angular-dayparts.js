    ...
    return {
        ...
        controller: ['$scope', '$element', '$attrs', function($scope, $element, $attrs) {
            ...
            function repopulate () {
                ...
                $scope.getdata = [];  
                for(var i=0;i<24;i++) { $scope.getdata[i] =[]; } 
                selected.forEach(function(item){
                    var getcount = item.split('-');
                    $scope.getdata[getcount[1]].push(getcount[0]);
                });
                
                $scope.getdata.forEach(function(item,key){
                    if(item.length==7){
                        var getid = parseInt(key)+1;
                        $element.find('#'+getid).addClass('red');
                    }                    
                });               
            }
            ...
            $scope.selectHour = function(hour) {
                ...
                $element.find('th').each(function(i, el){ 
                    if($(el).find('#'+newhour).hasClass('red')){
                        $(el).find('#'+newhour).removeClass('red');
                    }else {
                        $(el).find('#'+newhour).addClass('red');
                    }
                });
